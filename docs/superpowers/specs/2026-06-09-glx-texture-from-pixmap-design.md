...
- `synthesise_glx_fb_configs` (served at GET_FB_CONFIGS:8196) + `drawable_attributes_for` (already reports `GLX_TEXTURE_TARGET_EXT` / `GLX_Y_INVERTED_EXT` per the assessment): on depth-24/32 configs add `GLX_DRAWABLE_TYPE |= GLX_PIXMAP_BIT` and the bind-to-texture pairs **in Xorg's exact reply order** (glxcmds.c:1094-1100): `GLX_BIND_TO_TEXTURE_RGB_EXT`, `GLX_BIND_TO_TEXTURE_RGBA_EXT`, **`GLX_BIND_TO_MIPMAP_TEXTURE_EXT`** (=`0`/false — yserver doesn't back mipmap-from-pixmap, but the pair MUST be present to match the contract; was omitted), `GLX_BIND_TO_TEXTURE_TARGETS_EXT`, `GLX_Y_INVERTED_EXT`.
  - **RGB/RGBA values are yserver policy, not "Xorg-exact"** (only the reply *order* is exact): Xorg/glamor sets BOTH `bindToTextureRgb` and `bindToTextureRgba` true on its EGL-mirrored configs (glamor/glamor_glx_provider.c:175). yserver chooses depth-derived values — `RGB=true` on depth-24 (opaque; the sample_view already forces α=1.0 via the BgraNoAlpha swizzle), `RGBA=true` on depth-32 — which is stricter but matches the window's actual format the client picks the config by. Revisit to "both true" if a client rejects the stricter set.
...
- Promotion/export failure (no external-memory support / modifier query fails): fall back to linear tiling; if that fails, return **`BadPixmap`** on `BufferFromPixmap` — Xorg returns `BadPixmap` when `dri3_fd_from_pixmap()` fails (dri3/dri3_request.c:277); `BadAlloc` is used only later if *sending the fd to the client* fails. Compositor then degrades to its non-TFP fallback (current behavior — no worse than today).
...
- **The exact Vulkan↔dma-buf implicit-sync mechanism** (component 2). Concrete candidate path to validate (the standard Vulkan/Mesa dmabuf-interop pattern, e.g. wlroots / mutter-native): bridge Vulkan explicit sync ↔ the dmabuf's implicit reservation fences via **sync_file ioctls** —
  ...
  - *read→write:* before the next Vulkan write, snapshot the dmabuf's fences with **`DMA_BUF_IOCTL_EXPORT_SYNC_FILE` + `DMA_BUF_SYNC_WRITE`** (the WRITE flag waits on ALL current users — readers *and* writers; a READ-only export would miss active GL readers and let yserver overwrite a buffer muffin is still sampling), import as a Vulkan semaphore (`vkImportSemaphoreFdKHR` SYNC_FD), and wait on it in the submission.
