...
One sanity check: if the `LIBSEAT_BACKEND` environment variable is set to `direct`, we refuse to start with `ERROR yserver: LIBSEAT_BACKEND=direct refused; explicit operator misconfiguration`. This catches the case where an operator has explicitly told libseat to use the direct backend even when an IPC seat manager is available — which is almost always a mistake (loses logind's session isolation, device access mediation). It is a guardrail, not a security boundary; libseat's direct backend is not formally a privilege boundary either way.

If libseat auto-selects the direct backend (no logind on D-Bus, no `/run/seatd.sock`, no `LIBSEAT_BACKEND` env), we accept it. Whether libseat-direct succeeds depends on syscall-level permissions (file permissions on `/dev/dri/*` and `/dev/input/event*`, `CAP_SYS_TTY_CONFIG` for VT control, etc.) which the kernel enforces independently. If it succeeds, we gain VT switching; if it fails, we fall through to the legacy direct-open path.

When `libseat_open_seat` itself fails, fall back to the legacy direct-open path and log `INFO yserver: libseat unavailable (<errno>); VT switching disabled, opening devices directly`. The legacy path is what runs today — the existing bring-up scenarios (iMac 19,2, raw-tty experimentation) keep working.
...
Every wlroots+Vulkan compositor (sway, Hyprland, smithay derivatives) in production demonstrates this pattern working across logind / seatd / direct backends — because none of them need to actively keep Vulkan alive; the kernel's render-node design makes it automatic. The risk we still flag (Risks #9) is for hardware/drivers where Mesa is forced onto the primary node (no render node exposed); on those, master loss does affect Mesa, and we'd see `VK_ERROR_DEVICE_LOST` on the first post-resume submit. Modern KMS drivers (amdgpu, msm, i915, nouveau, asahi) all expose render nodes; the no-render-node case is an old / niche case we don't optimise for.
...
- Input-quiesce ack timeout (suggest ~100 ms).
- `vkQueueWaitIdle` timeout (reuse the existing 5s shutdown bound).
- Modeset timeout per CRTC (suggest ~1 s; kernel atomic commit typically completes in tens of ms).
- Resume input-thread `Resume` ack timeout (suggest ~100 ms).

With those bounds in place, the worst-case stall sums to ~6 s (one queue idle plus the various ack/modeset timeouts). The screen is going dark during the stall anyway; clients perceive a brief input lag at the VT-switch boundary, identical to what Xorg shows. If telemetry shows a typical switch exceeding ~250 ms we revisit (likely by moving the GPU wait to a worker thread and pumping the main loop), but the lean MVP path doesn't pay that cost upfront.
...
9. **Hardware / driver without a DRM render node forces Mesa onto the primary node.** On modern KMS drivers (amdgpu, msm, i915, nouveau, asahi) Mesa renders via `/dev/dri/renderD12N`, which doesn't participate in master — see "Device fd lifetime contract". Some old or niche drivers don't expose a render node; Mesa falls back to the primary node and *does* depend on master. On those, our `drmDropMaster` invalidates Mesa's context and we'd see `VK_ERROR_DEVICE_LOST` on the first post-resume submit. Mitigation: if telemetry shows this on a tested host (none expected in the matrix), we either exit-and-restart on resume or implement full Vulkan device-loss recovery as the follow-up.
