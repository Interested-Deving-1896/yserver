...
9. **Wlroots contract turns out to be backend-dependent.** wlroots's docstring documents the libseat contract, but the underlying behaviour comes from logind / seatd. In principle a future libseat backend could break the "DRM fd stays valid for non-master ops" assumption. Mitigation: if real `VK_ERROR_DEVICE_LOST` shows up on a tested backend after VT switch (logind on the daily-driver hosts), we discover quickly via telemetry and either narrow the supported backends or implement device-loss recovery.
