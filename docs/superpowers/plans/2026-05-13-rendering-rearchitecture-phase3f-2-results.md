...
- **T1 (`bc4f98e`)**: `MaskScratch::needs_image_grow(w, h) -> bool` accessor. Predicate: `w > self.extent.width || h > self.extent.height`. (Unlike `DstReadback::needs_grow` which has an unallocated-slot case because it allocates per-format on demand, `MaskScratch` always allocates its 256×256 image in `new()`, so there's no first-allocation branch to consider.) Mirrors `CopyScratch::needs_grow` from 3D and `DstReadback::needs_grow` from 3F-1 T1; used by T3 as the mask half of the combined resize-only pre-flush.
